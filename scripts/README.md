...
| 7 | 全量测试 + 覆盖率（n 个用例归属 §3.2） | 进程内 `run()` 跑 `tests/**/*.test.js`；**用例数 ≥ 1 否则 FAIL**（空匹配静默陷阱兜底）；覆盖率阈值按**每文件**判定，**仅限 `server/core` `server/ai` `shared` `cli` `public/js` 五目录**（P6 F0 并入前端）：行 ≥90 / 分支 ≥85 / 函数 ≥90，任一低于 → FAIL | P0-5 ✅ 实时（F0 起五目录） |
...
1. **`--test-coverage-*` CLI 阈值是聚合语义**（全文件合计），不是每文件；门禁项 7 自己实现**每文件**判定（仅五目录）。
...
**分层（按路径）**：`shared/*` L-1（唯一跨层共享）｜`server/data/*` 数据层（任何 server 模块可读）｜`server/core/{rng,field}` L0｜`server/core/{effects,items,unlock}` L1｜`server/core/{roles,skills,bullets}` L2｜`server/core/engine.js` L4｜`server/ai/*` L5｜`server/{index,ranked,runner,box,loadout,battle}.js` `cli/**` L6｜`public/**` L7（P6 前端）。
...
7. **L7 public（P6 F0）**：ESM import 扫描（`from '...'` 静态字面）——相对路径须可解析（unresolved）、`shared/*` 放行、**其余裸导入拒绝**（import-bare）；`public/**` 禁止 import `server/**`（public-server）；`require()` 扫描不适用于 public（ESM 下无该形态，动态 import() 盲区登记）。
