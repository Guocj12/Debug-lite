...
- [Debug-Lite AI 训练算法演进记录](#debug-lite-ai-训练算法演进记录)
  - [目录](#目录)
  ...
  - [6. 第四代（当前）：得分制奖励](#6-第四代当前得分制奖励)
    - [6.1 得分公式（零和）](#61-得分公式零和)
    - [6.2 两个关键性质](#62-两个关键性质)
    - [6.3 配套修复与优化](#63-配套修复与优化)
    - [6.4 观察记录：简单训练后 AI 已学会连招](#64-观察记录简单训练后-ai-已学会连招)
  - [7. 当前架构速览](#7-当前架构速览)
  - [8. 未来计划](#8-未来计划)
    - [8.1 核心思路：状态条件化基因](#81-核心思路状态条件化基因)
    - [8.2 落地路径](#82-落地路径)
    - [8.3 后续可选增强](#83-后续可选增强)
  - [9. 业界做法参考](#9-业界做法参考)
    - [9.1 核心概念：策略（Policy）](#91-核心概念策略policy)
    - [9.2 行业主流做法（按"如何优化策略"分三类）](#92-行业主流做法按如何优化策略分三类)
    - [9.3 对本项目的落地建议](#93-对本项目的落地建议)
    - [9.4 一句总结](#94-一句总结)
...
### 6.4 观察记录：简单训练后 AI 已学会连招

简单训练（默认参数，如 `--pop 80 --rounds 5`）后，从 `ai-weights.json` 的 Top 基因序列中已能观察到**有结构的连招行为**，而非零散的单次技能释放。最典型的是刺客的"绕背 combo"：

- **暗影步 → 匕首**（刺客）：序列中多次出现 `skill1(暗影步) → skill2(匕首)` 相邻编排，即瞬移到敌人身后、下一格接背刺真伤——这正是该职业设计的核心玩法。
- **突击盾 → 重击**（战士）：`skill3(突击盾) → skill2(重击)` 用 4 格冲刺贴身，接眩晕 2tick 的重击形成压制窗口。
- **远程 poke → 突进**（刺客）：序列开头先放 `skill3(毒瓶)` 远程消耗，中段再转入暗影步突进，形成"消耗 → 找机会"的节奏。

这些连招不是手写规则，而是双向对抗 + 得分制奖励下从对抗数据里**自然涌现**的——说明当前训练信号已经能区分"会连招"与"乱按技能"。这对后续状态条件化基因（见 §8）是有利的起点：连招是"序列内编排"，下一步是让 AI 学会"按局面选连招"。
...
一句话：**把"状态 → 行为"作为可学习对象（即策略 / Policy）**，是现代游戏 AI 的共同基石。你的"状态条件化基因"本质上就是一个策略，与行业主流方向一致，差别只在"用什么手段去优化这个策略"。

### 9.1 核心概念：策略（Policy）

> 策略就是函数 `π(a|s)`：输入局面 `s`，输出行动（或行动分布）。它天然满足"不同状态下同一套参数输出不同行为"——这正是 8.1 想表达的东西。把问题建模成"每个决策点看状态再决策"（即**马尔可夫决策过程 MDP**），训练复杂度就从"整条序列的组合"降为"状态 × 单步行动"，从而**正规地规避了序列长度的指数爆炸**——你 8.1 的直觉与 MDP 的因子化思路殊途同归。

### 9.2 行业主流做法（按"如何优化策略"分三类）

| 类别 | 代表方法 | 特点 | 与本项目的关系 |
|------|---------|------|---------------|
| **强化学习（梯度）** | DQN / Rainbow、PPO、A2C | 需要神经网络 + 反向传播 + 大量自对弈样本；PPO 是自对弈对战的事实标准（OpenAI Five 打 Dota 2） | 升级路径：把策略换成小网络，用 PPO 自对弈训练 |
| **自对弈 + 树搜索** | AlphaZero（MCTS + 策略/价值网络） | 靠"想得更深"取胜；适合可模拟、信息较完全的环境；对同时出手/双盲需做对手建模适配 | 终极路径：最强棋类战绩，但工作量最大 |
| **进化策略 / 神经进化** | NEAT、CMA-ES、OpenAI ES | 不用梯度，直接进化策略参数；实现简单、适合小状态空间 | **最贴近现状**：你的"GA + 状态条件化基因"正是这一类——用遗传算法进化一个策略，而非固定脚本 |

### 9.3 对本项目的落地建议

- **当前路径（低成本，先做）**：把 8.2 的"局面桶 → 序列"当作一个**表格式策略**进化——即行业里的表格策略/可解释策略，状态少、够用、零外部依赖。
- **升级路径（更强）**：把策略换成小神经网络（输入状态特征，输出 16 格序列或逐格行动），训练用 PPO 自对弈，或继续用 GA/ES 进化网络权重。状态空间变大后，神经策略的表达力明显强于查表。
- **终极路径（最强）**：AlphaZero 式 MCTS + 策略/价值网络，并对同时行动做对手建模；等 AI 进入"高手"阶段再考虑。

### 9.4 一句总结

你的方向（让基因"看状态"）在行业里就是**策略学习**，而且你已经正确避开了序列搜索的指数爆炸。先按 8.2 用查表策略 + 进化落地，跑出效果后再考虑神经网络 + PPO/自对弈把它做大——这基本就是行业从"脚本 AI"走向"学习型 AI"的标准路径。
