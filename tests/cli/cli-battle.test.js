...
test('CLI battle：--p1/--p2 对战 → 0；--out 落盘回放 JSON 且可读', async () => {
  await withServer(null, async ({ baseUrl }) => {
    ...
    const a = await quiet(() => cli.main(['battle', '--p1', LD_FILE, '--p2', LD_FILE, '--seed', '7', '--tier', 'mythic', '--out', out], { baseUrl }));
    ...
    fs.unlinkSync(out);
    const noOut = await quiet(() => cli.main(['battle', '--p1', LD_FILE, '--p2', LD_FILE, '--seed', '7'], { baseUrl }));
    assert.equal(noOut, 0, '无 --out 也成功');
  });
});

test('CLI battle 失败路径：参数/文件/业务错误 → 2/1', async () => {
  await withServer(null, async ({ baseUrl }) => {
    const noArg = await quiet(() => cli.main(['battle', '--p1', LD_FILE], { baseUrl }));
    assert.equal(noArg, 2, '缺 --p2 → 2');
    const miss = await quiet(() => cli.main(['battle', '--p1', path.join(__dirname, 'no.json'), '--p2', LD_FILE], { baseUrl }));
    assert.equal(miss, 2, '文件缺失 → 2');
    const badSeed = await quiet(() => cli.main(['battle', '--p1', LD_FILE, '--p2', LD_FILE, '--seed', 'x'], { baseUrl }));
    assert.equal(badSeed, 1, '非法 seed → 服务端 400 → 1');
    const badTier = await quiet(() => cli.main(['battle', '--p1', LD_FILE, '--p2', LD_FILE, '--tier', 'platinum'], { baseUrl }));
    assert.equal(badTier, 1, '非法 tier → 1');
    // P2-7 补充分支：p2 非法 loadout → 1；未知旗标 → 2；--out 写入失败 → 1
    const badLd = path.join(__dirname, '..', 'fixtures', 'loadout-bad.json');
    const badP2 = await quiet(() => cli.main(['battle', '--p1', LD_FILE, '--p2', badLd], { baseUrl }));
    assert.equal(badP2, 1, 'p2 非法 loadout（技能 2 个）→ 1');
    const flag = await quiet(() => cli.main(['battle', '--p1', LD_FILE, '--p2', LD_FILE, '--bogus'], { baseUrl }));
    assert.equal(flag, 2, '未知旗标 → 2');
    const writeFail = await quiet(() => cli.main(['battle', '--p1', LD_FILE, '--p2', LD_FILE, '--out', path.join(os.tmpdir(), 'no-such-dir-xyz', 'r.json')], { baseUrl }));
