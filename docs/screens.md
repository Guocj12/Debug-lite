# 前端场景示意图（7 屏）

> ⚠️ **已废弃（2026-09-15）**：本文是旧"绝对坐标盒模型"（`frontend-spec.md` v2）的布局快照，**坐标口径已作废**——v3 重设计改为浏览器正常布局（块流 + flex/grid），不存在统一坐标表。
> ⚠️ **2026-09-16 补充（D-129…D-136）**：服务端新增账号/档案/战绩/积分后，前端还需**登录注册屏、我的战绩（含防守战绩）、排行榜、配置槽管理（≤3 套，唯一出战）**等界面；这些尚未进入任何前端文档，清单见 `docs/systems/11-account-store.md` 附录 B。本文将不再补充这些屏。
> **现行唯一依据：`docs/frontend-spec.md` v3**（§10 逐屏布局与按钮表）；本文仅作历史留档，**不得**再作为布局测试的期望快照（旧轮"按坐标表实现 → 盒子堆在左上角/互相遮挡"的根因就在这里）。
> 生成：2026-09-13（临时渲染器，删除后本文件内置结果）
> **约定**：比例 = 每字符 8×16px；视口 1280×720 → 160 列 × 46 行；坐标口径与旧版 `docs/frontend-spec.md` §6 一致（**历史留档，非期望快照**——现行布局测试**不得**引用本文坐标，见上方两条横幅）。
> 图中盒子为"示意级"（小盒可能比实际略粗）；**精确坐标见下方每个场景的盒子表**。

## 图例

- `+` 边框角、`-`/`|` 边框、`#` 填充（小盒）；框内大写字母为组件标签（如 `AI EDIT`）。
- z 层：1 basemap/header → 2 面板 → 3 组件 → 4 控件 → 90+ 遮罩/弹窗。
- 战斗画布坐标为引擎 px（1px=1 逻辑像素，缩放 s=1）。

---

## 主菜单 menu

```
..+--------------------+........................................................................................................................................
..+--------LOGO--------+...............................................................................................................##TIER###.#####SEED#####.
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
.............................................+--------------------------------------------------------------------+.............................................
.............................................|####################################################################|.............................................
.............................................|#####+--------------------------------------------------------+#####|.............................................
.............................................|#####+---------------------Debug-Lite v3----------------------+#####|.............................................
.............................................|####################################################################|.............................................
.............................................|########################+------------------+########################|.............................................
.............................................|########################+-----AI EDIT------+########################|.............................................
.............................................|####################################################################|.............................................
.............................................|####################################################################|.............................................
.............................................|########################+------------------+########################|.............................................
.............................................|########################+----WAREHOUSE-----+########################|.............................................
.............................................|####################################################################|.............................................
.............................................|########################+------------------+########################|.............................................
.............................................|########################+------GACHA-------+########################|.............................................
.............................................|####################################################################|.............................................
.............................................|########################+------------------+########################|.............................................
.............................................|########################+------BATTLE------+########################|.............................................
.............................................|####################################################################|.............................................
.............................................|########################+------------------+########################|.............................................
.............................................|########################+-----SETTINGS-----+########################|.............................................
.............................................|####################################################################|.............................................
.............................................+--------------------------------------------------------------------+.............................................
...................................................###########################hint###########################...................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
```

| id | x | y | w | h | z |
|---|---|---|---|---|---|
| `logo` | 16 | 9 | 180 | 46 | 1 |
| `tierBadge` | 1080 | 20 | 72 | 24 | 1 |
| `seedLit` | 1160 | 20 | 112 | 24 | 1 |
| `panel_menu` | 360 | 180 | 560 | 360 | 2 |
| `title` | 408 | 216 | 464 | 32 | 3 |
| `btn_ai` | 560 | 268 | 160 | 40 | 4 |
| `btn_wh` | 560 | 320 | 160 | 40 | 4 |
| `btn_gacha` | 560 | 372 | 160 | 40 | 4 |
| `btn_battle` | 560 | 424 | 160 | 40 | 4 |
| `btn_settings` | 560 | 476 | 160 | 40 | 4 |
| `hint` | 408 | 540 | 464 | 24 | 4 |

---

## AI 编辑器 editor

```
..+--------------------+........................................................................................................................................
..+--------LOGO--------+...............................................................................................................##TIER###................
................................................................................................................................................................
................................................................................................................................................................
+-------------++------------------------------------------------------------------------------------------------------------------------------++---------------+
|#############||##+-----------------------+###################################################################################################||+------------+#|
|#############||##|######while(true)######|###################################################################################################||+--VALIDATE--+#|
|#############||##+-----------------------+###################################################################################################||+------------+#|
|#############||##############################################################################################################################||+--COMPILE---+#|
|#############||##############################################################################################################################||###############|
|#############||##############################################################################################################################||+------------+#|
|#############||##############################################################################################################################||+--TRY-RUN---+#|
|#############||##############################################################################################################################||###############|
|#############||##############################################################################################################################||+------------+#|
|#############||##############################################################################################################################||+--HASH...---+#|
|#############||##############################################################################################################################||###############|
|#############||##############################################################################################################################||###############|
|#############||######################################################BLOCKLY WORKSPACE#######################################################||####ERR 0######|
|#############||##############################################################################################################################||###############|
|#############||##############################################################################################################################||###############|
|#############||##############################################################################################################################||###############|
|#############||##############################################################################################################################||###############|
|##TOOL-BOX###||##############################################################################################################################||#####PANEL#####|
|#############||##############################################################################################################################||###############|
|#############||##############################################################################################################################||###############|
|#############||##############################################################################################################################||###############|
|#############||##############################################################################################################################||###############|
|#############||##############################################################################################################################||###############|
|#############||##############################################################################################################################||###############|
|#############||##############################################################################################################################||###############|
|#############|+------------------------------------------------------------------------------------------------------------------------------+|###############|
|#############|+------------------------------------------------------------------------------------------------------------------------------+|###############|
|#############||##############################################################################################################################||###############|
|#############||##############################################################################################################################||###############|
|#############||##############################################################################################################################||###############|
|#############||##########################################################ERROR LIST##########################################################||###############|
|#############||##############################################################################################################################||###############|
|#############||##############################################################################################################################||###############|
|#############||##############################################################################################################################||###############|
|#############||##############################################################################################################################||###############|
+-------------++------------------------------------------------------------------------------------------------------------------------------++---------------+
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
```

| id | x | y | w | h | z |
|---|---|---|---|---|---|
| `logo` | 16 | 9 | 180 | 46 | 1 |
| `tierBadge` | 1080 | 20 | 72 | 24 | 1 |
| `toolbox` | 0 | 64 | 120 | 592 | 2 |
| `workspace` | 120 | 64 | 1024 | 432 | 2 |
| `loop_forever` | 144 | 88 | 200 | 48 | 3 |
| `panel_right` | 1144 | 64 | 136 | 592 | 2 |
| `btn_validate` | 1156 | 80 | 112 | 32 | 3 |
| `btn_compile` | 1156 | 120 | 112 | 32 | 3 |
| `btn_run` | 1156 | 160 | 112 | 32 | 3 |
| `hash` | 1156 | 216 | 112 | 40 | 3 |
| `errCount` | 1156 | 272 | 112 | 24 | 3 |
| `errors` | 120 | 496 | 1024 | 160 | 2 |

---

## 仓库与装配 warehouse

```
..+--------------------+........................................................................................................................................
..+--------LOGO--------+...............................................................................................................##TIER###................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
..+-------------------+..+-------------------+--+-------------------+--+-------------------+--+-------------------+-----------------+..+---------------------+..
..|###################|..|###################|##|###################|##|###################|##|###################|#################|..|#####################|..
..|###################|..|#######CARD1#######|##|#######CARD2#######|##|#######CARD3#######|##|#######CARD4#######|#################|..|#####################|..
..|###################|..|###################|##|###################|##|###################|##|###################|#################|..|#####################|..
..|###################|..|###################|##|###################|##|###################|##|###################|#################|..|#####################|..
..|###################|..+-------------------+##+-------------------+##+-------------------+##+-------------------+#################|..|#####################|..
..|###################|..|##########################################################################################################|..|#####################|..
..|###################|..+-------------------+##+-------------------+###############################################################|..|#####################|..
..|###################|..|###################|##|###################|###############################################################|..|#####################|..
..|###################|..|#######CARD5#######|##|#######CARD6#######|###############################################################|..|#####################|..
..|###################|..|###################|##|###################|###############################################################|..|#####################|..
..|###################|..|###################|##|###################|###############################################################|..|#####################|..
..|###################|..+-------------------+##+-------------------+###############################################################|..|#####################|..
..|###################|..|##########################################################################################################|..|#####################|..
..|###################|..|##########################################################################################################|..|#####################|..
..|ROLE|SKILL|RPLG|SPL|..|################################################ITEM GRID#################################################|..|#######DETAIL########|..
..|###################|..|##########################################################################################################|..|#####################|..
..|###################|..|##########################################################################################################|..|#####################|..
..|###################|..|##########################################################################################################|..|#####################|..
..|###################|..|##########################################################################################################|..|#####################|..
..|###################|..|##########################################################################################################|..|#####################|..
..|###################|..|##########################################################################################################|..|#####################|..
..|###################|..|##########################################################################################################|..|#####################|..
..|###################|..|##########################################################################################################|..|#####################|..
..|###################|..|##########################################################################################################|..|#####################|..
..|###################|..|##########################################################################################################|..|#####################|..
..|###################|..|##########################################################################################################|..|#####################|..
..|###################|..|##########################################################################################################|..|#####################|..
..|###################|..|##########################################################################################################|..|#####################|..
..|###################|..|##########################################################################################################|..|#####################|..
..+-------------------+..+----------------------------------------------------------------------------------------------------------+..+---------------------+..
................................................................................................................................................................
..+----------------------------------------------------------------------------------------------------------------------------------------------------------+..
..+------------------------------------------------------------------------POINTS BAR------------------------------------------------------------------------+..
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
```

| id | x | y | w | h | z |
|---|---|---|---|---|---|
| `logo` | 16 | 9 | 180 | 46 | 1 |
| `tierBadge` | 1080 | 20 | 72 | 24 | 1 |
| `buckets` | 16 | 80 | 168 | 500 | 2 |
| `grid` | 200 | 80 | 864 | 500 | 2 |
| `card1` | 200 | 80 | 168 | 108 | 3 |
| `card2` | 384 | 80 | 168 | 108 | 3 |
| `card3` | 568 | 80 | 168 | 108 | 3 |
| `card4` | 752 | 80 | 168 | 108 | 3 |
| `card5` | 200 | 204 | 168 | 108 | 3 |
| `card6` | 384 | 204 | 168 | 108 | 3 |
| `detail` | 1080 | 80 | 184 | 500 | 2 |
| `points` | 16 | 592 | 1248 | 40 | 2 |

---

## 开箱 gacha

```
..+--------------------+........................................................................................................................................
..+--------LOGO--------+...............................................................................................................##TIER###................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
..................................................+----------------------------------------------------------+..................................................
..................................................|##########################################################|..................................................
..................................................|##+----------------------------------------------------+##|..................................................
..................................................|##+---------------------TIER: rare---------------------+##|..................................................
..................................................|##########################################################|..................................................
..................................................|##########################################################|..................................................
..................................................|##+----------------------------------------------------+##|..................................................
..................................................|##+----------------------TIMES: 3----------------------+##|..................................................
..................................................|########################G A C H A#########################|..................................................
..................................................|##########################################################|..................................................
..................................................|###################+------------------+###################|..................................................
..................................................|###################+-----OPEN BOX-----+###################|..................................................
..................................................|##########################################################|..................................................
............+-------------------+--+-------------------+--+-------------------+--+-------------------+---------------------------------------------+............
............|###################|##|###################|##|###################|##|###################|#############################################|............
............|########R1#########|##|########R2#########|##|########R3#########|##|########R4#########|#############################################|............
............|###################|##|###################|##|###################|##|###################|#############################################|............
............|###################|##|###################|##|###################|##|###################|#############################################|............
............+-------------------+##+-------------------+##+-------------------+##+-------------------+#############################################|............
............|######################################################################################################################################|............
............|###############################################################RESULTS################################################################|............
............|######################################################################################################################################|............
............|######################################################################################################################################|............
............|######################################################################################################################################|............
............|######################################################################################################################################|............
............|######################################################################################################################################|............
............|######################################################################################################################################|............
............+--------------------------------------------------------------------------------------------------------------------------------------+............
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
```

| id | x | y | w | h | z |
|---|---|---|---|---|---|
| `logo` | 16 | 9 | 180 | 46 | 1 |
| `tierBadge` | 1080 | 20 | 72 | 24 | 1 |
| `panel_gacha` | 400 | 210 | 480 | 300 | 2 |
| `sel_tier` | 424 | 250 | 432 | 40 | 3 |
| `fld_times` | 424 | 306 | 432 | 40 | 3 |
| `btn_open` | 560 | 368 | 160 | 40 | 4 |
| `results` | 96 | 420 | 1088 | 240 | 2 |
| `res1` | 96 | 420 | 168 | 108 | 3 |
| `res2` | 280 | 420 | 168 | 108 | 3 |
| `res3` | 464 | 420 | 168 | 108 | 3 |
| `res4` | 648 | 420 | 168 | 108 | 3 |

---

## 对战配置 battle

```
..+--------------------+........................................................................................................................................
..+--------LOGO--------+...............................................................................................................##TIER###................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
..+------------------------------------------------------------------------------+...+-----------------------------------------------------------------------+..
..|##############################################################################|...|#######################################################################|..
..|##+-------------------------------------------------------------------------+#|...|#######################################################################|..
..|##+-----------------------------OPPONENT: kiter-----------------------------+#|...|##+-----------------------------------------------------------------+##|..
..|##############################################################################|...|##|#################################################################|##|..
..|##############################################################################|...|##|#################################################################|##|..
..|##+-------------------------------------------------------------------------+#|...|##|#################################################################|##|..
..|##|#########################################################################|#|...|##|#################################################################|##|..
..|##|#########################################################################|#|...|##|#################################################################|##|..
..|##|#############################LOADOUT SUMMARY#############################|#|...|##|#################################################################|##|..
..|##|#########################################################################|#|...|##|############################STATS...#############################|##|..
..|##|#########################################################################|#|...|##|#################################################################|##|..
..|##+-------------------------------------------------------------------------+#|...|##|#################################################################|##|..
..|##############################################################################|...|##|#################################################################|##|..
..|##+---------------------------------+##+-------------+########################|...|##|#################################################################|##|..
..|##+---------SEED: 20260912----------+##+---RANDOM----+########################|...|##|#################################################################|##|..
..|##############################################################################|...|##|#################################################################|##|..
..|##############################################################################|...|##+-----------------------------------------------------------------+##|..
..|##############################################################################|...|#######################################################################|..
..|##############################################################################|...|#######################################################################|..
..|##############################################################################|...|#######################################################################|..
..|##############################################################################|...|#######################################################################|..
..|##############################################################################|...|#######################################################################|..
..|##############################################################################|...|#######################################################################|..
..+------------------------------------------------------------------------------+...+-----------------------------------------------------------------------+..
................................................................................................................................................................
......................................................................+------------------+......................................................................
......................................................................+------START-------+......................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
```

| id | x | y | w | h | z |
|---|---|---|---|---|---|
| `logo` | 16 | 9 | 180 | 46 | 1 |
| `tierBadge` | 1080 | 20 | 72 | 24 | 1 |
| `config` | 16 | 80 | 640 | 400 | 2 |
| `sel_opp` | 40 | 120 | 600 | 40 | 3 |
| `loadoutSum` | 40 | 176 | 600 | 120 | 3 |
| `fld_seed` | 40 | 312 | 280 | 40 | 3 |
| `btn_seed` | 336 | 312 | 120 | 40 | 3 |
| `preview` | 680 | 80 | 584 | 400 | 2 |
| `stats` | 704 | 140 | 536 | 240 | 3 |
| `btn_start` | 560 | 500 | 160 | 40 | 4 |

---

## 回放与结算 replay

```
..+--------------------+........................................................................................................................................
..+--------LOGO--------+...............................................................................................................##TIER###................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
..############----------------------------------------------------------------------------------------------------############---+...+-----------------------+..
############################+------+################################################################+------+#####################|...|#######################|..
..|#########################|##P1##|#################################################+--+###########|##P2##|#####################|...|#######################|..
..|#########################|######|#######################BATTLE CANVAS#############+--+###########|######|#####################|...|#######################|..
..|#########################+------+################################################################+------+#####################|...|#######################|..
..|##############################################################################################################################|...|#######################|..
..|##############################################################################################################################|...|#######################|..
..+------------------------------------------------------------------------------------------------------------------------------+...|#######################|..
..+------------------------------------------------------------------------------------------------------------------------------+...|#######################|..
..|##########################################PLAY PAUSE STEP SPEED ---SLIDER--- T:5/18###########################################|...|#######################|..
..+------------------------------------------------------------------------------------------------------------------------------+...|#######################|..
.....................................................................................................................................|#######################|..
..................................................+----------------------------------------------------------+.......................|#######AI TRACE########|..
..................................................|##########################################################|.......................|#######################|..
..................................................|##########################################################|.......................|#######################|..
..................................................|##########################################################|.......................|#######################|..
..................................................|######################RESULT: DRAW?#######################|.......................|#######################|..
..................................................|##########################################################|.......................|#######################|..
..................................................|##########################################################|.......................|#######################|..
..................................................|##########################################################|.......................|#######################|..
..................................................|##########################################################|.......................|#######################|..
..................................................+----------------------------------------------------------+.......................|#######################|..
.....................................................................................................................................|#######################|..
.....................................................................................................................................|#######################|..
.....................................................................................................................................+-----------------------+..
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
```

| id | x | y | w | h | z |
|---|---|---|---|---|---|
| `logo` | 16 | 9 | 180 | 46 | 1 |
| `tierBadge` | 1080 | 20 | 72 | 24 | 1 |
| `canvas` | 16 | 80 | 1024 | 128 | 1 |
| `base_r` | 992 | 102 | 32 | 26 | 2 |
| `base_l` | 0 | 102 | 32 | 26 | 2 |
| `p1` | 224 | 96 | 64 | 64 | 3 |
| `p2` | 800 | 96 | 64 | 64 | 3 |
| `bullet` | 686 | 112 | 32 | 32 | 3 |
| `hp1` | 16 | 84 | 96 | 8 | 4 |
| `hp2` | 912 | 84 | 96 | 8 | 4 |
| `controls` | 16 | 220 | 1024 | 56 | 2 |
| `aiTrace` | 1064 | 80 | 200 | 400 | 2 |
| `modal` | 400 | 280 | 480 | 160 | 90 |

---

## 设置 settings

```
..+--------------------+........................................................................................................................................
..+--------LOGO--------+...............................................................................................................##TIER###................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
..+--------------------------------------------------------------------+...+---------------------------------------------------------------------------------+..
..|####################################################################|...|#################################################################################|..
..|##+---------------------------------+###############################|...|##+-------------+################################################################|..
..|##+--------------SEED---------------+###############################|...|##+----LEVEL----+################################################################|..
..|####################################################################|...|#################################################################################|..
..|####################################################################|...|#################################################################################|..
..|##+---------------------------------+###############################|...|##+---------------------------------------------------------------------------+##|..
..|##+-----------TIER: rare------------+###############################|...|##|###########################################################################|##|..
..|####################################################################|...|##|###########################################################################|##|..
..|####################################################################|...|##|#################################CHANNELS##################################|##|..
..|##+--------------------------------------------------------------+##|...|##|###########################################################################|##|..
..|##|##############################################################|##|...|##|###########################################################################|##|..
..|##|##############################################################|##|...|##|###########################################################################|##|..
..|##|##########################ABOUT TEXT##########################|##|...|##+---------------------------------------------------------------------------+##|..
..|##|##############################################################|##|...|#################################################################################|..
..|##|##############################################################|##|...|#################################################################################|..
..|##|##############################################################|##|...|##+---------------------------------------------------------------------------+##|..
..|##+--------------------------------------------------------------+##|...|##|###########################################################################|##|..
..|####################################################################|...|##|###########################################################################|##|..
..|####################################################################|...|##|################################RING BUFFER################################|##|..
..|####################################################################|...|##|###########################################################################|##|..
..|####################################################################|...|##|###########################################################################|##|..
..|####################################################################|...|##+---------------------------------------------------------------------------+##|..
..|####################################################################|...|#################################################################################|..
..+--------------------------------------------------------------------+...+---------------------------------------------------------------------------------+..
................................................................................................................................................................
..+--------------------------------------------------------------------+........................................................................................
..|##+-------------+##+-------------+##################################|........................................................................................
..|##+---EXPORT----+##+---IMPORT----+VE################################|........................................................................................
..|####################################################################|........................................................................................
..+--------------------------------------------------------------------+........................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
................................................................................................................................................................
```

| id | x | y | w | h | z |
|---|---|---|---|---|---|
| `logo` | 16 | 9 | 180 | 46 | 1 |
| `tierBadge` | 1080 | 20 | 72 | 24 | 1 |
| `left` | 16 | 80 | 560 | 400 | 2 |
| `fld_seed` | 40 | 120 | 280 | 40 | 3 |
| `sel_tier` | 40 | 176 | 280 | 40 | 3 |
| `about` | 40 | 240 | 512 | 140 | 3 |
| `logPanel` | 600 | 80 | 664 | 400 | 2 |
| `sel_level` | 624 | 120 | 120 | 40 | 3 |
| `channels` | 624 | 176 | 616 | 140 | 3 |
| `ring` | 624 | 348 | 616 | 120 | 3 |
| `save_row` | 16 | 500 | 560 | 80 | 2 |
| `btn_export` | 40 | 516 | 120 | 40 | 3 |
| `btn_import` | 176 | 516 | 120 | 40 | 3 |

---

