...
- **现状证据**
  - `server/ai/runtime.js:53-68` `getPath(snap, path)`：路径正则 `path.match(/^([\w]+)(?:\[(\d+)\])?(?:\.([\w]+))?$/)`（`:55`）—— `\w` **允许任意键名**，包括 `__proto__`、`constructor`、`prototype`；取值走 `snap[m[1]]`（`:57`）与 `v[m[3]]`（`:64`），**不做 `hasOwnProperty` 检查、不走白名单**（对比 `lookupVar` 在 `:73` 就用了 `Object.prototype.hasOwnProperty.call`）。
  - 当前之所以没形成实际读取面，是因为**上游唯一快照来源是白名单投影**：`server/runner.js:44-55` `projectSnapshot` 每 tick 新建**只含固定字段**的对象（`pick()` 在 `:47` 只取 `hp/atk/def/sp/mp/x/baseHp/facing`；顶层 `:49-54` 只有 `self/enemy/bullets/field`），并先 `deepFreeze`（`runtime.js:26-32`）。调用点：`server/runner.js:118`、`server/battle.js:71`、`server/ranked.js:68`。
  - 也就是说：**这是"靠调用方自觉"的边界**，运行时本身没有任何白名单（`docs/systems/08-ai.md` §4.5 的"只读快照"语义在运行侧未强制）。
- **风险**：① 若将来快照加入**对象字段**（如 `self.effects`、`bullets[i].meta`、新增 `self.buffs`），`get` 路径 `x.y` 即可沿原型链读到非预期属性（如 `snap.self.constructor`、`__proto__` 上的成员），轻则信息泄漏（把内部结构泄给玩家程序），重则影响判定的取值语义；② 一旦有第二处调用 `resume` 时用了**非投影**的状态对象（例如为了性能直接传 `state`），漏洞立即变为可利用。
- **建议处置方向**：**运行侧也加白名单**（`getPath` 只允许投影契约声明的键集合，逐段校验；显式拒绝 `__proto__`/`constructor`/`prototype`；对象段用 `hasOwnProperty` + `Object.isFrozen` 检查）；在 `docs/systems/08-ai.md` §4.5 写明"投影白名单 + 运行时白名单"双保险；补一条测试：路径含危险键名时返回 0。
- **优先级**：**中**
- **状态**：待处理
...
| SEC-17 | AI 运行时 `get` 路径解析放宽正则，仅靠投影兜底 | D | 中 | 待处理 |
| SEC-18 | 依赖与供应链：无锁文件，`node_modules` 含 extraneous 包 | D | 中 | 待处理 |
| SEC-19 | 无 CI / 无自动化门禁执行 | D | 中 | 待处理 |
