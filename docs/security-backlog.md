...
### SEC-33 回放帧**永远返回双方 `aiTrace`** → 对手 AI 的逐步决策（执行路径与条件值）对玩家可见（用户 2026-09-25 裁定接受）

- **现状证据**（2026-09-25 实测）：`GET /api/v1/replay/:id` 的帧里 `diff.aiTrace` **同时包含 `owner=p1` 与 `owner=p2`** 的条目；条目形如 `{tick, owner, seq, path:'body.s[1].else.s[0]', nodeType:'if'|'action', phase:'eval', depth, result}` —— 即**对手 AI 执行了哪些节点、走了哪个分支、最终产出了什么动作**。
  - 修前口径（§9.4 / P1-1/P1-2）是"**默认只给请求方自己一侧**，`?trace=all` 需管理员令牌"；**`D-167` 明确推翻了它**（用户口径："回放帧永远返回双方 ai，前端绘制时只画我方 ai"）。
- **风险**：① **AI 逻辑可被反推**——玩家的 AI 就是其"代码"，轨迹暴露了阈值/分支结构（例如"血量 < 30 就走 else 分支开火"），对手可据此针对性编写克制 AI；② 现有 `?trace=all` 的管理员门控随之失去意义（不再需要令牌）。
  - **边界**：不外泄的是**程序源码**（`GET /me/ai` 只返回自己的库；对手的 `program`/`programHash` 从不出现在任何玩家侧响应里），故这是"**行为级**泄露"而非"源码级"泄露；且按 D-130/§15.1 的既有结论，段位/积分本就"不具备竞技可信度"。
- **处置**：**用户 2026-09-25 知情后裁定接受风险并登记（record-only，`D-167`③）**。若日后要收紧，可选：① 恢复按 side 裁剪（前端只画自己一侧即可，裁剪不影响任何计算）；② 仅对 **bot 对手**返回双方轨迹；③ 对局结束后延迟 N 小时再开放对手轨迹。
- **优先级**：**中**（对外部署且玩家在意 AI 私密性时为**高**；本机/自娱部署可忽略）
- **状态**：**待处理（用户裁定接受风险，2026-09-25）**

---

### SEC-34 取消对手匹配的 24h 硬底线 → 薄池下可**反复匹配同一对手**（可刷分 / 刷晋升；用户 2026-09-25 知情后接受）

- **现状证据**（2026-09-25 实测）：`D-168` 把 D-136 的"24h 硬底线（间隔 <24h 两池皆拒）+ strict/relaxed 双池"改为**软冷却**：候选**永不因冷却被硬拒**，只按 `clamp(已过小时/4, 0, 1)` 加权轮盘抽签。实测：**12 个候选里有 11 个"已恢复"时，刚打过的那个权重 0 者不会被抽中**；但**当池子里所有人都"刚打过"（权重全 0）时**，会退化为"最久未打"一组内抽签并**照常打满 10 场**（`tests/unit/ranked.test.js` T-RK-4b）；两轮批次之间也不再有任何"对手不可用"的等待（`tests/integration/e2e-play.test.js` E2E-5：第二轮同样 6 场，而非旧口径的 0 场）。
- **风险**：① 池子薄（尤其单机自测/小规模部署）时，同一对手可被反复抽中 ⇒ **排位晋升（10 场胜 >6）可被"挑一个弱对手连刷"**；② 快速对战积分同理（非对称 Elo 会随积分差收敛，但仍可在低分段反复吃同一对手）；③ "同批次内不重复"只限制**批内**，不限制**跨批**。
- **边界**：`D-152`（禁 bot 充数）、`batchId = f(playerId, seed)` 幂等、`store.abuse.suspect` 审计、`dailyBattleLimit` 参数（**已留未启用**）仍在；`POST /ranked/run` 不接受客户端 `pool`（`pool_forbidden`）⇒ 只能"被动接受服务端抽到的重复"，不能自选对手。
- **处置**：**用户 2026-09-25 知情后裁定接受风险并登记（record-only，`D-168`④）**。若要收紧，可选：① 恢复一个**较短**的硬窗（如 30 分钟）；② 启用 `dailyBattleLimit`；③ 对"同对手每日场次"设上限；④ 加权时再乘一个"近 N 场内已交手次数"的衰减因子。
- **优先级**：**中**（薄池 + 排行榜对外可见时为**高**）
- **状态**：**待处理（用户裁定接受风险，2026-09-25）**
...
| SEC-33 | 回放帧**永远返回双方 `aiTrace`** → 对手 AI 的逐步决策可见（行为级泄露；程序源码不外泄） | A | 中（对外部署且在意 AI 私密性 → 高） | **待处理（用户 2026-09-25 知情后裁定接受风险并登记，`D-167`③，record-only）**（收紧选项：按 side 裁剪 / 仅 bot 给双方 / 延迟开放） |
| SEC-34 | 取消对手匹配的 **24h 硬底线**（改 4h 线性软冷却）→ 薄池下可反复匹配同一对手（可刷分/刷晋升） | B | 中（薄池 + 排行榜对外 → 高） | **待处理（用户 2026-09-25 知情后裁定接受风险并登记，`D-168`④，record-only）**（收紧选项：短硬窗 / 启用 `dailyBattleLimit` / 同对手每日上限 / 近 N 场衰减） |
| | **合计** | | **高 8 / 中 19 / 低 7 = 34**（2026-09-25 新增 SEC-32：高 +1；SEC-33/SEC-34：中各 +1） | **已处置 7 / 已部分处置 8 / 待处理 19**（2026-09-25 更新） |
