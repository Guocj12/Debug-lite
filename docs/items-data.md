...
| 特殊 | 持续伤害 | 命中持续伤害 3 tick（**hp 类资源流量：结算即生效、到期不回滚**） | mp +2/+4/+6 | 特殊效果：持续伤害 |
...
| 特殊 | 释放增益 | 释放后 atk **每 tick +2**（`duration=2` 共结算 2 次，生效窗口 t+1/t+2）；**到期回滚累计增量、净 0**（不再是永久增益） | mp +2/+4/+6 | 特殊效果：释放增益（**释放即入队、下一 tick 起效**，D-70；面板类到期回滚见 `systems/05-effects.md` §4.3） |
