...
- **收口实跑（2026-09-25，D-164…D-167）**：`npm test` = **1073 通过 / 0 失败**（1058 → 1073：+7 镜像、+5 回放/bot、+3 内联帧）；`npm run gate` = **9 PASS / 0 FAIL / 0 PEND**（项7 digest `551cf8db9af2`）；`check-docs` = **PASS**（批次计数仍 **41**）；`check-arch` = **PASS**（40 文件）。**去 flaky**：`RP-7` 曾因"第三个账号注册在匹配之前 → 候选池 3 人"而间歇失败（已修正注册顺序 + 固定 publicId/playerId）；api 套件连跑 3 次全绿。
- **⏳ 本批剩余（未做）**：4 小时线性软冷却（作废 D-136 硬底线 + 删 `relaxed`）、`winner` 对外统一 `p1/p2/draw/invalid`、`POST /admin/account-patch`。**这三项落地前 F6/F7 不启动。**
