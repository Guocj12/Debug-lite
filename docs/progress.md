...
- **收口实跑（2026-09-25，本轮已完成的三个决策）**：`npm test` = **1070 通过 / 0 失败**（1058 → 1070：+7 镜像、+5 回放/bot）；`node scripts/check-docs.js` = **PASS**（批次计数仍 **41**）；`node scripts/check-arch.js` = **PASS**（40 文件）。**夹具迁移**：`tests/cli/cli-replay.test.js` 的 p2 追击夹具按新契约改写为 p1 帧（`move_left` → `move_right`，**未放宽任何断言**）。
- **⏳ 本批剩余（未做）**：帧契约瘦身（去 `events`、保留画面字段与**双方** `aiTrace`、补五维/行动/buff/基地受击/弹幕生命周期/伤害数值）、quick·ranked 内联全量帧、`POST /admin/replay-frames` 调试帧接口、4 小时线性软冷却（作废 D-136 硬底线 + 删 `relaxed`）、`winner` 对外统一 `p1/p2/draw/invalid`、`POST /admin/account-patch`。**这些落地前，本批不提交、F6/F7 不启动。**
