...
- 实测（本轮）：`npm test` **943 通过 / 0 失败**；`npm run gate` **9 PASS / 0 FAIL / 0 PEND**（连跑两次）；`check-docs` PASS；`fe-spec-check` 9 PASS；`baseline --compare` 无新增失败（exit 0）；`npm run e2e` **22/22**；`npm run play` exit 0。
...
- 实测（UTF-8 收口后）：`npm test` = **946 通过 / 0 失败**；`npm run gate` = **9 PASS / 0 FAIL / 0 PEND**；`check-docs` PASS；`fe-spec-check` 9 PASS；`baseline --compare` exit 0（无新增失败）；`npm run e2e` **22/22**；`npm run play` exit 0。
