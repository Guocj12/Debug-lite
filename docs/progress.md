...
- **✅ D-171 排行榜扩展（已交付，2026-09-25）**：`GET /leaderboard` 支持**两张榜**（`order=points` 积分榜 = 既有口径；`order=arrival` **段位榜** = `tier` 高→低 → 同段位 `tierUpdatedAt` 升序）+ **分页**（`offset`，响应 `total`/`hasMore`）+ **本人名次** `data.self`（可选 Bearer；匿名/坏 token → `null`）；行新增 `tierUpdatedAt`。实现唯一处 = `store.index.board`（`leaderboard()` 退化为薄封装）；索引条目补 `tierUpdatedAt`，**老 `index.json` 缺该字段 → 开机从档案重建补齐**（info 留痕 + 立即落盘）。**证据**：`tests/api/api-leaderboard.test.js` LB-3…LB-7（分页一致性 / 本人名次 / 段位榜排序 / 参数负例 / 老索引迁移）+ `tests/contract/store-contract.test.js` CN-12。
- **✅ F7 锦标赛屏 + 排行榜屏（已交付，2026-09-25）**：分册 `docs/frontend/05-tournament.md`。锦标赛：批次汇总（`matches/requested`·胜负平无效·段位前后·晋升·奖励品质）、**缺口诚实行**（`shortfall>0` 时写明"缺场批次不判晋升"）、**10 场分页**（每页 5 场，翻页零请求）、每场 `看这一场`（有内联帧直接用；**重放批次的场次自动读回放**；无效场按钮禁用并写明原因）、战斗查看器与 AI 逻辑查看器**复用 F6 的 `state.viewer` 与投影**。排行榜：积分榜/段位榜 + 六个范围按钮（`data-tier` 寻址）+ 分页 + **本人名次行**（`self=null` 时如实写明"未登录或未上榜"）。新增 10 个动作 + `state.tournament`/`state.board` 两个切片。**证据**：`tests/frontend/tournament-board-flow.test.js` TB-1…TB-10（真实 HTTP）。**未完成项：浏览器人工走查**（05 分册 §11 的 13 步）→ 待用户执行，记录落 `docs/reviews/F7.md`。
- **✅ 独立审查与修复轮（2026-09-25）**：F6/F7 各由**独立上下文子代理**出具对抗式审查记录（`docs/reviews/F6.md` 354 行 / `docs/reviews/F7.md` 219 行；探针一律留 `%TEMP%`，仓库零改动）。**F6 判定 ③④ FAIL**（4 条 P1 + 8 条 P2）、**F7 判定 ② FAIL**（2 条 P1 + 3 条 P2），已逐条修复并补回归（F6→**QB-11/QB-12** + QB-5/6/7/8 强化；F7→**LB-8/LB-9/TB-11/TB-12** + TB-8 死分支激活），修复回执见两份审查记录的末节（F6 §6 / F7 §8）。其中两条是**真缺陷**：① 榜单缓存漏失效 `banned` ⇒ 被封禁者仍留在榜上（与同一响应的 `self=null` 自相矛盾，违反 `systems/11` §8.6）；② 迁移探针把"档案缺字段"误判为"索引缺字段" ⇒ 每次开机全量重建索引且永不收敛（现改为"只认缺键" + 读档用 `createdAt` 一次性兜底盖章）。
- **✅ 本轮全部任务已交付**：后端 `D-164…D-171`、前端批次 `F6`/`F7`。**待用户验收**：F6 §11（12 步）+ F7 §11（13 步）的真实浏览器人工走查。
