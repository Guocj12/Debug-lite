...
6. **消耗补偿**：除减耗类外，每个插件把消耗增量叠加到 `cost` 声明维度（D-113）。**口径（B20 定稿，P2-1）**：每声明维度 **delta = costDeltaBase[插件品质] × 插件档位 tier**（如 rare tier1 = 3×1 = mp+3，I-6d）；数据表 `costDeltaByTier` 的数组值（如 `{mp:[2,4,6]}`，common 基准名义值）仅作**维度声明**、不参与计算——逐档期望值由公式导出（rare=[3,6,9]、epic=[4,8,12]…）。
