...
| 11 | 超时扣血 | `tick ≥ 48` → 双方基地与角色**同时**扣血，各自按**自身 `maxHp`**：角色扣 `ceil(角色.maxHp × 0.0625)`，基地扣 `ceil(基地.maxHp × 0.0625)`（2026-09-16 修正，见 §4.6） |
...
3. `tick ≥ 48` 起每 tick 双方基地与角色同时扣血，**各自按自身 `maxHp`**：角色 `ceil(角色.maxHp × 0.0625)`、基地 `ceil(基地.maxHp × 0.0625)`；`6.25% × 16 = 100%`，故**必在 64 tick 前结束**。
   - **2026-09-16 修正（用户拍板 A，"变强即变弱"缺陷）**：此前基地被扣的份额取自**角色** `maxHp`（`state.bases[o].hp -= ceil(players[o].maxHp × ratio)`），而基地 `maxHp` 固定 100、角色 `maxHp` 随品质涨到 ~180 → **角色越强，自己基地越先被打空**：实测 hp 180 对 hp 100 的纯对峙，强者必败。现改为基地按**基地自身 `maxHp`** 结算（`battle-config.json` 的 `bases.*.maxHp`；缺该字段时回退 `bases.*.hp`）。
   - 该修正只影响 `tick ≥ 48` 的战斗；`hardCapTick=64` 与 `overtimeStart=48` 不变。
- 优先级：基地 > 角色；同级同时 → 平局。
...
`T-EN-1..10`、`T-BT-1..29`（帧可重建状态、数值边界、顺序无关、64 tick 上界、同 seed 一致、背击×暴击=2.25、防御单调、弹幕上界、事件顺序、命中幂等、超时精确（`T-EN-4b`/`T-BT-11b`：角色按角色 `maxHp`、**基地按基地自身 `maxHp`**）、位移不越界、黄金复现、走查可复算、碰撞伤害、基地规则、碰撞精度、AOE 用位移后位置、弹幕不跨 tick；含 D-72 位移三态免疫、`turn` 朝向、`cast_buff` 下一 tick 起效、`hp_regen` 逐 tick 回复）。
