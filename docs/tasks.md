...
**F7 锦标赛屏 + 排行榜屏（2026-09-25 起）**：分册 `docs/frontend/05-tournament.md`。**含本批唯一后端契约扩展 D-171**（`GET /leaderboard`：两张榜 + 分页 + 本人名次；索引补 `tierUpdatedAt` + 老索引开机迁移）。前端新增：`tournament` 真屏（批次汇总 / 缺口诚实行 / 10 场分页 / 每场「看这一场」/ 复用 F6 的查看器）、`leaderboard` 真屏（积分榜/段位榜 + 六范围 + 分页 + 本人名次）、10 个动作、`data-tier` 寻址（`render.targetAttrs` + `app.payloadOf`）；机器核对 `tests/frontend/tournament-board-flow.test.js` TB-1…TB-10 + `tests/api/api-leaderboard.test.js` LB-3…LB-7。**未完成项：浏览器人工走查**（分册 §11 的 13 步）→ 待用户执行，记录落 `docs/reviews/F7.md`。**F7 不新增批次号**（§6 仍为 41 批）。
