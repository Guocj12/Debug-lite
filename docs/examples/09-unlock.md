...
| 段位 | 新解锁 | 累计可用节点数（示例表） |
|---|---|---|
| **绿 common** | `if/else`（条件分支）、均衡模板、基础技能 | **11** |
| **蓝 rare** | `loop / while / break`（循环）、特化模板、进阶技能 | **14** |
| **紫 epic** | `random`（概率随机）、`logic` 与扩展运算符、进阶技能（**D-120 已确认**） | **17** |
| **橙 legendary** | 专家模板、高阶技能（**不新增语法节点**） | **17** |
| **青 mythic** | `function / call`（函数）、顶级技能 | **19** |

> 节点数只统计语法节点；`legendary` 不新增语法（所以与 epic 同为 17），其解锁体现在**模板/技能池**。
...
| U-1c | `availableNodes('mythic')` 是否含全部 | ✅ 19 个 |
...
| # | 行为 | 期望 |
|---|---|---|
| U-6a | 工具箱渲染 | 只列出 `availableNodes(tier)` 的积木 |
