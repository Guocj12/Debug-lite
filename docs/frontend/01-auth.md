...
|---|---|---|---|---|
| `submit-login` | 登录屏·`登录` | `POST /auth/login` | 写入会话 → 切 `home`，显示 `登录成功：<昵称>（<账号>）` | 停留登录屏，`#notice` 显示服务端文案（如 `用户名或密码错误`） |
| `submit-register` | 注册屏·`注册` | 客户端预校验 → `POST /auth/register` | 写入会话 → 切 `home`，显示 `注册成功：<昵称>（<账号>）` | 停留注册屏，`#notice` 显示服务端文案（如 `用户名已被占用（大小写不敏感）`） |
...
| `submit-password` | 设置密码屏·`提交改密` | 客户端预校验（新密码两次一致）→ `POST /auth/password` | 停留本屏，显示 `改密成功：已撤销其他设备会话 <n> 个`（`n` 取 `revokedOthers`） | 停留本屏，`#notice` 显示 `原密码错误` / 密码规则文案 |
...
| `invalid_credentials` | 401 | 登录用户名/密码错；改密原密码错 | login、password | 无（服务端已区分 `用户名或密码错误` / `原密码错误`） | 无 |
...
| B-7 | `fetch` 抛错（服务未启动 / 断网） | `#notice` 显示 `无法连接服务器：<原因>`，**保留**已填表单 |
