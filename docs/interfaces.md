...
| `core/unlock.js` | `tierIndex` / `isUnlocked` / `filterByTier` / `validateLoadout` / `availableNodes`（**validateAi 于 B13 退役**，AI 程序校验由 ai/ast.validate 统一承担）。**`availableNodes(tier)` 只返回真实节点类型**（`ai-nodes.json` 的 `nodes`，17 类；`unlock.json` 的 `nodePermissions` 负责权限名→节点展开：`while` 折叠 `loop`、`implemented:false` 的 `arith_ext` 不授予）；段位累计数 11/13/15/15/17 | L1 + unlock.json/ai-nodes.json | B4 ✅（2026-09-16 口径收口） |
...
| GET | `/api/v1/unlock?tier=` | 该段位可用节点/模板/技能（`nodes` 只含真实节点类型） | 400 `bad_tier` | B4 ✅ 已实现 |
| POST | `/api/v1/box` | 开箱（seed/tier/次数；tier **缺省 common**；段位序号即品质上限 D-122 且截断后按剩余池重归一；409 `tier_locked` = 门控后掉落池为空——当前数据防御路径；seed 缺省生成并回带） | 400 / 409 `tier_locked` | B17 ✅ 已实现 |
