...
| POST | `/api/v1/me/configs` | 新建配置槽（**D-160 起建空槽**，不再复制出战配置） | 401；409 `slot_limit` | B29 ✅ / **D-160 语义变更** |
| PUT | `/api/v1/me/configs/:slotId` | 保存配置。**D-160**：非出战槽**允许不完整**（200，`snapshot:null`、`complete:false`、`missing:[…]`，loadout 正文随 journal 落盘）；出战槽不完整 → 409（逐位置 details）。完整时冻结快照（D-131） | 401；404 `slot_not_found`；409 `loadout_invalid`/`config_conflict` | B29 ✅ / **D-160 语义变更** |
| POST | `/api/v1/me/configs/:slotId/activate` | 设为出战配置。**D-160**：**此时**校验完整性（角色 + 恰 3 技能 + AI；允许插槽为空）→ 不完整 409 `cannot_activate_incomplete`；完整但缺快照自愈冻结 | 401；404 `slot_not_found`；409 `cannot_activate_incomplete` | B29 ✅ / **D-160 语义变更** |
...
| **GET** | **`/api/v1/me/warehouse`** | **D-159 真源**：`{buckets, usage, caps, counts, starterIssued}`；`usage[uid].slotIds[]` = 该物品被哪些出战配置引用（同物品可多配置引用） | 401 | 🆕 **D-159 新增语义** |
...
| **POST** | **`/api/v1/me/box`** | **D-159 服务端权威开箱**（需 Bearer）：体 `{tier?,times?}`（**D-162：无 `seed` 入参**）→ 物品**直接入档**并回带 `{seed,tier,times,items,counts,caps,grantId}`；任一桶超上限 → 409 且不入档 | 400 `bad_json`/`bad_tier`/`bad_times`；401；409 `warehouse_full` | 🆕 **D-159/D-162 新增** |
...
9. **Snapshot（D-135）**：`{hash, engineVersion, dataVersion, configHash, loadout, frozenAt}`；内容寻址存于 `runtime/snapshots/`，不可变，冻结后深拷贝 + canonical hash。**D-159 起**：服务端已有整仓，快照自带的 `warehouse`（装配引用子集，缺口 1 的时代产物）不再是必需来源，仅作兜底。
...
- **D-159/D-161/D-162 实例级覆盖缝（✅ 已接线；契约的一部分，不是内部细节）**：`start({boxSeed})` = 开箱 seed 的确定性序列（第 n 次 = `boxSeed+n−1`；D-162，缺省 `null` → `crypto.randomInt`）；`start({replayLimit})` / `start({config:{replayCacheSize}})`（既有）；`start({publicDir})`（F1）。数值上限 `warehouse.maxPerBucket`（500）与 `ai.maxPerPlayer`（100）在 `server/data/service-config.json`，schema 冻结比对（门禁项 4）。
