...
| `server/index.js` | `/api/v1`（§2）；**P7-4 新增**：Bearer 鉴权中间件（`Authorization: Bearer <token>`）、回放 LRU 64 + 参与者鉴权 + `410 replay_expired`、`DL_LEGACY_STATELESS` 双轨分派、`readBody` 超限 → `413 payload_too_large`、CORS 白名单。**P1 缺口 1/2 接缝（2026-09-19；D-159/D-163 修订）**：`rt.snapshotWarehouseOf(playerId)`（读出战快照自带的装配引用子集）、`rt.loadWarehouse(playerId)`（**四来源**：**服务端权威仓库** → account 镜像 → 进程内缓存 → 快照自带）、`rt.rememberWarehouse` | L6 | P0-8 / P7-4 ✅ |
...
| `server/store/ledger.js` | journal 记录构造与幂等 apply：`buildBattleRecord` / **`buildBatchRecord`（含 `invalids`(int)；D-168 起不再写 `relaxed`）** / `buildPromoteRecord` / `buildConfigRecord` / `buildRemovedRecord` / `battleIdOf` / 非对称 Elo 纯函数（`expectedScore`/`gainFactor`/`lossFactor`/`ratingDelta`/`settleRating`/`promoteAfterBatch`） | L6（store 内） | B27 / B32 ✅ |
| `server/runner.js` | `compileAi(program, logger)` / `runAiBattle({program,seed,tier,opponent,logger})` / `projectSnapshot(state,owner)` / **守方镜像（D-164）**：`mirrorSnapshot(snapshot)` / `unmirrorAction(action)` / `makeAiDriver(built)` / `MIRROR_ACTION` / `OPPONENTS` / `baselinePlayer` | L6 | B16 ✅ / D-164 ✅ |
| `server/box.js` | `openBoxes({seed,tier,times,items?,logger})`（校验 + 每箱独立 rng 流 + 409 映射）/ `BOX_TIMES_MAX` | L6 | B17 ✅ |
| `server/loadout.js` | `EMPTY_LOADOUT` / `validateLoadout(loadout,{warehouse,tier})`（I-12 全案 + T-PB-9 引用完整 + T-PB-8 双引用 + 门控）/ `buildPanel`（五维/regen/special/技能参数聚合）/ `resolveItems(loadout,warehouse)`（D-163 权威仓库解析）/ `referencedUidsOf` / **`warehouseResolves(loadout,warehouse)`（D-165：仓库覆盖判据，与 `resolveItems` 同谓词——角色+3 技能+全部插件引用都要在库）**。**2026-09-16 修复**：面板投影白名单补 `specials`/`castEffects`/`affixes`，并叠加角色插件 `hp_regen/sp_regen/mp_regen` 到 `regen`（此前 API 路径会静默丢失这些机制） | L6 | B19 / D-163 / D-165 ✅ |
| `server/ranked.js`（P7-3 改造；D-168 软冷却） | **实际实现（2026-09-25 实测）**：`takeSnapshot` / `runRankedBattle({loadout?,warehouse?,pool?,seed?,tier?,store?,playerId?})`（**档案驱动**：服务端抽池 + 双向记账；D-132/D-152；**D-168 软冷却** = `cooldownWeightOf`/`pickByCooldownWeight`/`drawByCooldown`/`recoveryHoursOf`/`lastFaceAtOf`，取代 D-136 的 `splitByCooldown`/`COOLDOWN_RELAX_MULT`/`cooldownHoursOf`）/ `promote(tier,wins)`（**只判定不落盘**；晋升在 `/ranked/run` 内落地）/ `tierReward` / `promotedAt` / `battleOne` / `batchIdOf` / `X_PROMOTE` / `TIERS`。**不存在** `submitLoadout`；`pool` 入参 → 400 `pool_forbidden`。池不足如实回报 `shortfall` | L6 + store + engine | P5 ✅ / B31 ✅ / D-168 ✅ |
| `server/store/*`（L6；✅ 已实现，B27 / **D-159/D-161/D-170/D-171 扩展**） | 工厂 `createStore({logger,dataDir,adapter,config,ratingConfig,versions,now})` / `openStore`；适配器契约（json 与 sqlite 逐项等价）：生命周期 `open/close/isOpen`；档案 `loadArchive/saveArchive/updateArchive/listPlayerIds/getSummary`；账号 `createAccount/setPasswordHash/setBanned/setNickname/setPool/touchLastSeen/markRecordsSeen`；**D-170** `accountPatch`；配置槽 `saveConfigSlot/createConfigSlot/activateConfigSlot/deleteConfigSlot/freezeSnapshot`；**D-159 仓库** `getWarehouse/grantBox/applyWarehouseChange`；**D-161 AI 库** `listAi/createAi/deleteAi`；journal `append/appendMany/applyRecord/applyRecords/settleBattle/readRecords/findBattleRecord/replayJournal/maxSeq/compactJournal`；`index.{snapshot,get,byTier,leaderboard,board,needsTierStamp,rank,rebuild,save,stats}`（**D-171**：`board` 是榜单唯一实现，含两张榜/分页/本人名次）；`snapshot.{freeze,put,get,has,list,ref,refCount,gc,stats}`；`sessions.{put,get,touch,revoke,revokePlayer,list,prune,size}`；维护 `recover/rebuildIndex/gc/stats`。**唯一允许 `node:fs` 的目录**；json 适配器实现原子写/journal 分段+group commit/物化档案/内容寻址快照库/单进程锁/崩溃恢复五步/迁移钩子（**v1→v2**：补空仓库与空 AI 库；**D-171**：索引缺 `tierUpdatedAt` → 开机从档案重建补齐） | L6 | B27 ✅ / **D-159/D-161/D-170/D-171 🆕**（15 文件） |
...
| `server/account.js`（L6；✅ 已实现，B29/B30 / **D-159/D-160/D-161 扩展**） | `createAccount({store,logger?,now?})` → `getSummary/listConfigs/createSlot/saveConfig/activateConfig/deleteSlot/saveWarehouseMirror/getWarehouseMirror/records({playerId,since?,limit?,role?})/markSeen/defenseSummary/setNickname/createPlayerArchive` + **D-159** `getWarehouse/assemblePlugin/disassemblePlugin` + **D-161** `listAi/createAi/deleteAi`；纯函数 `defaultLoadout/validateLoadoutOf/validateWarehouseMirror/ok/fail/statusOf/detailOf/toFailure`。`records` 返回 `records/since/latestSeq/limit/role/unread/maxSeq`（`nextSince` 已改名 **`latestSeq`**，游标推进只由 `records/seen` 负责）。**D-159**：`createPlayerArchive` 无显式 loadout 且非 bot 时走 **starter**（并建满 3 槽），引用校验默认用**服务端仓库**；`saveWarehouseMirror` 退役为"只校验形状"（引用不覆盖 → 200 + `verified:false`）。**D-160**：`saveConfig` 对非出战槽放宽完整性、`activateConfig` 才校验完整性。**D-161**：AI 库名称 1~24 字符、`program.type='program'`，删除被出战配置引用者 → 409 | L6 + store | B29/B30 ✅ / **D-159…D-161 🆕** |
...
| `server/admin.js`（L6；✅ 已实现，B33 / **F2 扩展**） | `createAdmin({store,env,logger,now,adminUsers?,resolveUsername?})` → `injectDebugBots/clearDebugBots/rebuildIndex/stats/ban`（`ban` 覆盖封禁与解封，写 journal `account.banned/account.unbanned`）+ **F2 新增** `accounts`（分页全量账号列表）/ `deleteAccount`（写 `player.removed` 墓碑）/ `checkAccess` / `isAdminPlayer`；模块级导出 `adminUsersOf(env)`。**访问判定（D-158）**：管理员账号（`DL_ADMIN_USERS`）**或**令牌（`DL_ADMIN_TOKEN`；缺失 → 503 `admin_token_missing`，不匹配 → 403 `forbidden`）；`DL_DEBUG_BOTS` 仅对 `bots` 构成第二道门控；`tokenEquals` 用 `crypto.timingSafeEqual` | L6 + store | B33 ✅ / F2 ✅ |
| `server/starter.js`（L6；✅ 已实现，**D-159 新增**） | `buildStarter({publicId?,playerId?,logger?})` → `{ok, seed, warehouse, loadout, aiLibrary, stats}`；纯函数 `starterSeedOf(publicId,playerId)`（`sha256('starter|publicId|playerId')` 前 8 hex → 合法 seed 区间）/ `pluginPoolFor(kind,slotTypes)` / `freeSlotIndexOf(item,plugin)`。**生成路径复用 `core/items`**（不手写物品对象）；1 角色（`role_bal`，common → **必有 ≥1 插槽**）+ 3 技能（`skill_melee_whirl`/`skill_straight_precise`，重掷至至少 1 个技能有槽，上限 20）+ 1~2 角色插件 + 1 技能插件（**按实际槽类型筛池并 `assemble` 装上 → 不会 slot_type_mismatch**）；AI 复用 `ranked.buildDefaultLoadout().ai` 并登记为库内一条（`name='新手AI'`）。可复现粒度 = **内容级**（物品 uid 由 `core/items` 进程内计数器分配，不参与内容级比较，B17 口径） | L6 + core/items + ranked | **D-159 🆕** |
| `cli/index.js` | 子命令（§3）；**P7-4 新增** `auth register\|login\|logout\|change-password` / `me` / `quick run` / `leaderboard` / `ranked promote`；**退出码 3 = 未鉴权**；token 来源 `--token` > `options.token` > `DL_TOKEN`。**只走 HTTP 不 require core**（L14）；**D-162：`box` 不再接受 `--seed`** | L6 | P0-8 / P7-4 ✅ |
...
| POST | `/api/v1/box` | 开箱（**遗留无状态路径**；tier **缺省 common**；**门控默认关闭** → tier 不再起门控作用、品质取自全池 `dropRates`）。**D-162 起无 `seed` 入参**：客户端传了被**忽略**，seed 一律服务端生成（响应回带，仅供审计/复现日志）；HTTP 侧确定性由 `start({boxSeed})` 注入缝提供。**本路径不入档** | 400 `bad_tier`/`bad_times`/`bad_json`；409 `tier_locked`（门控后掉落池为空，防御路径） | B17 ✅ / **D-162 语义变更** |
| GET | `/api/v1/warehouse` | 仓库规范骨架（**遗留**：服务端恒返回空骨架，仅供取桶键契约） | — | B18 ✅ 已实现 |
| POST | `/api/v1/warehouse/assemble` | 装配（**遗留纯函数路径**：请求内传整仓、不落盘） | 409 `slot_type_mismatch`/`points_exceeded`/`slot_occupied`/`tier_locked`/`plugin_equipped`/`item_missing` | B18 ✅ 已实现 |
| POST | `/api/v1/warehouse/disassemble` | 拆卸（**遗留纯函数路径**） | 404 `slot_empty`/`plugin_missing` | B18 ✅ 已实现 |
...
| POST | `/api/v1/ranked/run` | 排位：**双轨（P7-4）**。① 有 Bearer token → **档案驱动**：服务端抽池（`byTier ∩ 可用快照 ∩ 未封禁 ∩ 在池 ∩ 排除自己`）、**软冷却加权轮盘抽签**（D-168：`weight = clamp(已过小时/opponentRecoveryHours,0,1)`，**永不因冷却硬拒**；全员权重 0 时取最久未打一组）、双向记账（发起者 attack 同步结算；防守方离线只记 `defense`，**不掉段不掉分**）、池不足**如实回报 `shortfall`**（**禁止 bot 充数**，D-152）、`batchId = f(playerId,seed)` 幂等、晋升在此落地。② 无 token 且 `DL_LEGACY_STATELESS=1`（默认）→ 遗留无状态（`loadout/warehouse/pool/tier` 由请求传入）；`=0` → 401。**D-167**：`results[].frames` **内联该场完整战斗过程**（画面数据 + 双方 `aiTrace`，不含日志；幂等重放场次为 `null`，按 `battleId` 走 `GET /replay/:id`）。**D-168**：响应含 `recoveryHours`；**不再有 `relaxed`** | 400 `bad_seed`/`bad_tier`/`pool_forbidden`（**传入 `pool` → 服务端抽池不接受**）/`bad_pool`（遗留路径）；401 `unauthorized`；409 `no_loadout`/`loadout_invalid`/`no_active_config`/`store_not_found`；503 `store_unavailable` | B24 ✅ / B31 ✅（D-132） / D-167 ✅ / D-168 ✅ |
...
| GET | `/api/v1/me/configs` | 3 套配置全文（`slots[].loadout` + `snapshot` + `activeSlotId` + `maxSlots` + `unverifiedLoadout`） | 401 | B29 ✅ / **D-160 起注册即 3 槽** |
| POST | `/api/v1/me/configs` | 新建配置槽（**D-160 起建空槽**，不再复制出战配置）。**D-163**：可带 `loadout`（必须完整）——同样按 uid 从服务端仓库解析并受"一件物品只属于一份配置"约束 | 401；409 `slot_limit`/`loadout_invalid`/`item_in_use` | B29 ✅ / **D-160 + D-163 语义变更** |
| PUT | `/api/v1/me/configs/:slotId` | 保存配置。**D-160**：非出战槽**允许不完整**（200，`snapshot:null`、`complete:false`、`missing:[…]`，loadout 正文随 journal 落盘）；出战槽不完整 → 409（逐位置 details）。完整时冻结快照（D-131）。**D-163**：物品按 uid 从**服务端仓库**解析（客户端正文的数值一律丢弃；uid 不在仓库 → 409 `loadout_invalid` `物品不在仓库: <uid>`）；同一份配置内全部 uid 必须唯一；一件物品**只能属于一份配置**（撞车 → 409 `item_in_use`）；请求里的 `warehouse` 镜像**不再被采纳** | 401；404 `slot_not_found`；409 `loadout_invalid`/`config_conflict`/`item_in_use` | B29 ✅ / **D-160 + D-163 语义变更** |
| POST | `/api/v1/me/configs/:slotId/activate` | 设为出战配置。**D-160**：**此时**校验完整性（角色 + 恰 3 技能 + AI；允许插槽为空）→ 不完整 409 `cannot_activate_incomplete`；完整但缺快照自愈冻结。**D-163**：再判一次**跨配置独占**（历史数据可能是在独占规则生效前存下的）→ 409 `item_in_use` | 401；404 `slot_not_found`；409 `cannot_activate_incomplete`/`item_in_use` | B29 ✅ / **D-160 + D-163 语义变更** |
...
| **GET** | **`/api/v1/me/warehouse`** | **D-159 真源**：`{buckets, usage, caps, counts, starterIssued}`；`usage[uid].slotIds[]` = 该物品被哪份出战配置引用（**D-163 起至多一项**：一件物品同时只能属于一份配置） | 401 | 🆕 **D-159 新增语义** |
| **POST** | **`/api/v1/me/warehouse/assemble`** | **D-159 服务端态装配**：体 `{targetUid,pluginUid,slotIndex}`（**不传整仓**）→ `{warehouse,usage,counts,caps}`；校验仍由 `core/items` 纯函数单点完成，落 journal 增量记录 | 400；401；409 `item_missing`/`slot_type_mismatch`/`slot_occupied`/`points_exceeded`/`plugin_equipped` | 🆕 **D-159 新增** |
| **POST** | **`/api/v1/me/warehouse/disassemble`** | **D-159 服务端态拆卸**：体 `{targetUid,slotIndex}` | 400；401；404 `slot_empty`/`plugin_missing` | 🆕 **D-159 新增** |
| PUT | `/api/v1/me/warehouse` | **D-159 起退役**：只做形状校验（形状非法仍 400）；引用不覆盖出战配置**不再 409**，改 200 + `verified:false`。回执两个字段**语义分叉且都必须回带**：`unverifiedLoadout` = 本次镜像是否覆盖（回执局部量）、`archiveUnverifiedLoadout` = 档案真源标志（与 `GET /me` 的 `flags.unverifiedLoadout` 同源；D-159 起镜像不覆盖不再改写真源） | 400 `bad_request` | B29 ✅ / **D-159 语义退役** |
| **POST** | **`/api/v1/me/box`** | **D-159 服务端权威开箱**（需 Bearer）：体 `{tier?,times?}`（**D-162：无 `seed` 入参**）→ 物品**直接入档**并回带 `{seed,tier,times,items,counts,caps,grantId}`；任一桶超上限 → 409 且不入档。**D-163**：若某件发放物品的 uid 与该仓库已有物品**撞车**（uid 计数器是进程级的，重启后会归零重发），发放路径在写 journal 前把它**重映射**为仓库内空闲 uid —— 故**响应 `items[].uid` = journal 记录 = 档案**（前端按它做 key 是安全的） | 400 `bad_json`/`bad_tier`/`bad_times`；401；409 `warehouse_full` | 🆕 **D-159/D-162/D-163 新增** |
| **GET** | **`/api/v1/me/ai`** | **D-161 AI 库列表**：`{items:[{aiId,name,program,createdAt,updatedAt}], count, max:100, usage}` | 401 | 🆕 **D-161 新增** |
| **POST** | **`/api/v1/me/ai`** | **D-161 命名保存**：体 `{name,program}`（名称 1~24；`program.type='program'`）→ `{aiId,ai,count,max}`；满 100 → 409 | 400 `bad_request`；401；409 `ai_limit` | 🆕 **D-161 新增** |
| **DELETE** | **`/api/v1/me/ai/:aiId`** | **D-161 删除**：回 `{deleted,referencedBy,count,max}`；**被出战配置引用 → 409** | 401；404 `store_not_found`；409 `ai_in_use` | 🆕 **D-161 新增** |
...
| POST | `/api/v1/quick/run` | 快速对战（积分窗口递进 + 非对称 Elo 双向结算，D-133）。**D-168**：对手选择 = **软冷却加权轮盘**（`weight = clamp(已过小时/opponentRecoveryHours,0,1)`，**永不因冷却硬拒**；全员 0 时取最久未打）；响应 `seed` = **对局种子**（入参 `seed` 只影响匹配抽选）、含 `opponentWeight`/`recoveryHours`，**不再有 `relaxed`**；**D-169**：`winner` 为**绝对口径** `p1\|p2\|draw`（与 `/ranked/run`、回放帧、journal 判决一致；档案里"我赢了几场"仍为 `win/loss/draw`）。**D-167**：响应内联 `data.frames` = 该场**完整战斗过程**（画面数据 + 双方 `aiTrace`，不含日志） | 400 `bad_seed`；401；403 `banned`；**404 `store_not_found`**；409 `no_opponent`（**仅当候选池确实为空/窗口用尽**时才出现）/`no_active_config`/`loadout_invalid`/`store_not_found`（旧文档口径）| B32 ✅ 已实现 / D-167 ✅ / D-168 ✅ / **F6 ✅（04 分册接 UI）** |
| GET | `/api/v1/leaderboard` | 排行榜（**D-171 扩展**）：`?scope=global\|tier:<t>&order=points\|arrival&offset=&limit=`。`order=points`（缺省）= **积分榜**（`points`→`peakPoints`→`updatedAt`）；`order=arrival` = **段位榜**（`tier` 高→低 → 同段位 **`tierUpdatedAt` 升序** → `publicId`）。`limit` 1..100、`offset` ≥0；响应 `{scope, order, offset, limit, total, hasMore, rows[], self}`，行 = `{rank, publicId, nickname, points, tier, tierUpdatedAt}`（`rank` 为**该榜该 scope** 内名次）；`self` = 调用者本人名次（**可选** Bearer 才有，`null` = 未登录/未上榜；不含 `playerId`）。**无需鉴权仍成立**（`tolerant`：带 Bearer 才认身份） | 400 `bad_scope`/`bad_request`；503 `store_unavailable` | B30/B32 ✅ / **D-171 ✅** |
...
>
> **D-160 配置完整性校验时机**：`PUT /me/configs/:slotId` 对**非出战槽**不校验完整性（不冻结快照、loadout 正文随记录落盘）；**出战槽**仍要求完整（角色 + **恰 3 技能** + AI，**允许插槽为空**）；`activate` 时校验完整性 → 409 **`cannot_activate_incomplete`**；`POST /me/configs` 建**空槽**。
>
> **D-162 开箱随机性收归服务端（修订 T-AP-5）**：`POST /api/v1/box` 与 `POST /me/box` **都没有 `seed` 入参**（客户端传了被忽略，不再有 `bad_seed`）；seed 一律服务端生成；HTTP 侧确定性由实例级注入缝 `start({boxSeed})` 提供（第 n 次 = `boxSeed+n−1`）。进程内 `server/box.js` 的 `openBoxes({seed})` 保留（离线 `play`/`demo`/单测不受影响）。

> **F2 管理面访问模型（D-158）**：`POST /api/v1/admin/:op` 的授权 = **① 请求者账号命中 `DL_ADMIN_USERS`（Bearer，无需令牌）** 或 **② 既有令牌路径**（`X-Admin-Token`/Bearer == `DL_ADMIN_TOKEN`）。两者皆不满足时：令牌未配置 → 503 `admin_token_missing`，不匹配 → 403 `forbidden`（**既有语义不变**）。`DL_DEBUG_BOTS=1` 仍只对 `/admin/bots` 构成第二道门控；`register`/`login` 回带 `data.player.isAdmin`，`GET /me` 回带 `data.flags.isAdmin`。**越权检查例外**：`runEntry` 的"请求体 `playerId` 必须与令牌一致"不施加于 admin 面（管理面以他人为操作对象是设计意图）。
...
| `pool_forbidden` | 400 | 排位请求传入 `pool`（服务端抽池，D-136/D-168） |
...
| `cannot_delete_self` | 409 | **F2**：管理员删除自己的账号（`POST /admin/delete-account`） |
| `warehouse_full` | 409 | **D-159**：某分类已达每桶上限 500（拒绝开箱；**不写 journal**） |
| `cannot_activate_incomplete` | 409 | **D-160**：设为出战时配置不完整（保存非出战槽时不校验） |
| `ai_limit` | 409 | **D-161**：AI 库已满（默认 100，与物品**分别计数**） |
| `ai_in_use` | 409 | **D-161**：删除被**出战配置**引用的 AI |
| `item_missing` / `slot_type_mismatch` / `slot_occupied` / `points_exceeded` / `plugin_equipped` | 409 | **D-159**：服务端态装配的拒绝码（`core/items` 单点判定；`message` 已改为玩家可读文案） |
| `item_in_use` | 409 | **D-163**：该物品已被**另一份配置**引用（一件物品同时只能装配到一份配置）；`details[].path` = 物品 uid，`message` 写明占用它的 slotId。前端在候选弹窗里**预先标灰**（`已被配置N使用…`），服务端仍是唯一判据 |
| `slot_empty` / `plugin_missing` | 404 | **D-159**：拆卸时槽为空 / 悬挂引用（`core/items` 口径） |
| `store_adapter_unavailable` | — | `DL_STORE=sqlite`（`open()` 抛错，不静默退回 json） |

## §3 CLI 契约 v1（本轮唯一"操作台"，L14）

```
box --tier common --times 10                 # D-162：**无 --seed**（seed 由服务端生成；给了即参数错误 exit 2）
...
3. **回放帧 `frame`（D-167 重构；画面数据自足）**：`{tick, diff}`，`diff` 字段如下——**`diff.players` 是对象 `{p1,p2}`（不是数组）**；位置/碰撞位置/命中位置/弹幕出现消失位置/伤害位置均 **1px 整数**。
   ...
   - `verdict`：`{winner:'p1'|'p2'|'draw', phase:'base'|'role'} | null`（**非终局帧为 `null`**）。<br>⚠️ **2026-09-25 实测更正（F6 §9.3）**：文档此前写作 `{winner, reason, phase, ticks}` —— 实测 `server/core/engine.js` 只写 `winner` + `phase`；`reason`/`ticks` 只存在于 **journal 判决**（`ranked.js` 的 `{winner, reason:null, ticks}`）与 `POST /battle` 摘要（`battle.js` 的 `{id, seed, tier, winner, phase, ticks}`）里。
   - `aiTrace[]`：**扁平数组**（**不是**按侧容器）—— 每条 `{tick, owner:'p1'|'p2', seq, path, nodeType, phase:'eval', depth[, result]}`（`result` **只有 `action` 节点**才有，值为该 action 名；`seq` 每 tick 从 0 重排，单 tick 上限 2000 条，超出静默截断）。D-167 起**永远双方都返回**，前端按 `owner` 自行分组（只画自己一侧）。<br>⚠️ **2026-09-25 实测更正（F6 §9.3）**：文档此前暗示"每条带 owner"却未写形状；F6 的 `contract.FRAME_TRACE_FIELDS` + `quick-battle-flow.test.js` QB-6/QB-7 把它机器化。
...
8. **PlayerArchive（D-129/D-131/D-159/D-161，✅ 已实现）**：`archiveVersion/playerId/publicId/nickname/auth/progress{tier,peakTier,lastBatchId}/rating{points,peakPoints,games,wins,losses,draws}/configs{slots[≤3],activeSlotId,activeSnapshotHash}/pool{inPool,enteredAt,lastDrawnAt,drawnCount,lastOpponentAt}/record{appliedSeq,recent[],stats{attack,defense},unread}/flags{banned,banReason,isBot,cheatSuspect,unverifiedLoadout,rebuiltFromCheckpoint}` + **`warehouse{buckets{role,skill,rolePlugin,skillPlugin},starterIssued,grantIds[{grantId,seq,at,count}]}`（D-159；每桶 ≤ `service-config.warehouse.maxPerBucket` = 500）** + **`ai{items[{aiId,name,program,createdAt,updatedAt}]}`（D-161；≤ `service-config.ai.maxPerPlayer` = 100）**。**`ARCHIVE_VERSION = 2`**（v1 → v2 由 `migrateV1toV2` 补**空**仓库与空 AI 库；**老账号不发 starter**）。字段全表详见 `11-account-store §5.2`（含**文档外补录 5 字段**：`auth.username`/`auth.usernameLower`、`progress.lastBatchId`、`flags.banReason`、`flags.rebuiltFromCheckpoint`、`pool.lastOpponentAt`）。`playerId` **不对外返回**（只暴露 `publicId`）；**每档案 `appliedSeq` ≠ 全局 `index.seq`**。**槽位不变量（D-160）**：必有出战配置；**只有出战槽必须有已冻结快照**，非出战槽允许无快照（`loadout` 正文随 `player.config.saved` 记录落盘）。
9. **Snapshot（D-135）**：`{hash, engineVersion, dataVersion, configHash, loadout, frozenAt}`；内容寻址存于 `runtime/snapshots/`，不可变，冻结后深拷贝 + canonical hash。**D-159 起**：服务端已有整仓，快照自带的 `warehouse`（装配引用子集，缺口 1 的时代产物）不再是必需来源，仅作兜底；**D-163 起 HTTP 保存路径不再写入该字段**（配置路由不传客户端镜像），只有服务端内部传仓库的路径（注册 starter）会带上。
...
11. **rating-config.json（D-133 + D-168，✅ 已存在）**：`{base:0,cap:3000,scale:400,kBase:32,kMin:8,kMax:64,drawFactor:0.5,matchWindowStart:100,matchWindowStep:100,matchWindowMax:600,`**`opponentRecoveryHours:4`**`,dailyBattleLimit:0,rounding:'half_up',promoteWins:6,batchSize:10}`。**表为数值单一来源**，代码默认值兜底 = `server/store/config.js` 的 `DEFAULT_RATING_CONFIG`，schema 冻结值 `RATING_CONFIG_FROZEN` + 跨字段不变量校验，**缺表必 FAIL**。**D-168**：`opponentRecoveryHours` = 对手软冷却"线性回满"小时数（**取代 D-136 的 `opponentCooldownHours` 24h 硬底线**：候选永不因冷却被硬拒，只按已过时间加权）。
12. **service-config.json（D-129/D-159/D-161，✅ 已存在）**：`{auth{...},session{...},config{maxSlots:3,slotIdPrefix},record{recentLimit:100},store{archiveCacheSize,snapshotCacheSize},journal{...},snapshot{retentionDays},replayCacheSize:64,pool{ttlDays,`**`opponentRecoveryHours:4`**`},warehouse{maxPerBucket:500},ai{maxPerPlayer:100}}`。**D-159/D-161 新增 `warehouse.maxPerBucket` 与 `ai.maxPerPlayer`**（服务端强制：超限拒绝开箱 / 拒绝创建 AI）；**D-168**：`pool.opponentRecoveryHours`（取代 `pool.opponentCooldownHours`，仅作形状校验，匹配侧真源是 `rating-config`）。`usernameMin/usernameMax/nicknameMax` **已被消费**（`nicknameMax` 在写入时夹到 ≤16）；`pool.ttlDays` 参数已留、**未启用**（默认 0 = 不过期）。校验与兜底同 §4.11。
...
| D-130 | §1 store/*（混合权威：仓库仍客户端 → **⚠️ 已由 D-159 推翻**：仓库/物品/装配改服务端权威） + §4.8 flags.unverifiedLoadout | | D-131 | §2 auth/register + §4.8 configs.slots[≤3] | | D-132 | §2 ranked/run（双向记账） + §4.8 record.stats.defense |
...
| D-157 | §2 ranked/run·quick/run（**匹配池筛选与最终实例化共用同一可用性判定**；不得用 bot 凑数，D-152） | | D-158 | §2 admin/*（管理面访问模型：`DL_ADMIN_USERS` 账号优先 + 令牌回落；`/admin/accounts`、`/admin/delete-account`）+ §2.1 `cannot_delete_self` + §7 `DL_ADMIN_USERS` | | D-159 | §1 store/*+`server/starter.js`（**仓库服务端权威，推翻 D-130**）+ §2 `me/warehouse*`/`me/box` + §4.8 `warehouse` 段/§4.12 `warehouse.maxPerBucket` + §5 作弊面关闭 |
| D-160 | §2 `me/configs`（**非出战槽允许不完整**；activate 才校验完整性）+ §2.1 `cannot_activate_incomplete` + §4.8 槽位不变量（只有出战槽必有快照） | | D-161 | §1 `server/account.js` AI 库门面 + §2 `me/ai*` + §2.1 `ai_limit`/`ai_in_use` + §4.8 `ai` 段/§4.12 `ai.maxPerPlayer` | | D-162 | §2 `box`/`me/box`（**无 `seed` 入参**，随机性收归服务端）+ §3 CLI `box` 去 `--seed` + §7 `start({boxSeed})` 注入缝 |
| D-163 | §1 `server/loadout.js` 的 `resolveItems`/`referencedUidsOf`（**物品身份/数值取自服务端仓库**）+ §2 `me/configs`（解析/同配置唯一/**跨配置独占**；不再采纳请求里的 `warehouse` 镜像）+ §2.1 `item_in_use` + §2 `me/box`（发放 uid 冲突重映射）+ §4.8 `usage.slotIds` 至多一项 |
...
| D-171 | §2 `GET /leaderboard`（**两张榜 + 分页 + 本人名次**：`order=points\|arrival`、`offset`/`total`/`hasMore`、行加 `tierUpdatedAt`、`data.self`；`tolerant` 认可选 Bearer）+ §1 `server/store/index-file.js` 的 `board`/`ensureArrivalSorted`/`needsTierStamp` + §1 `server/store/adapter-json.js`（`index.board` + open() 的 `tierUpdatedAt` 补齐迁移）+ §2.1 复用 `bad_request`/`bad_scope` |
...
| L6 | store | `store.open/close`(info) / `store.read`(trace) / `store.write`(debug) / `store.journal.append`(debug) / `store.journal.flush`(trace) / `store.journal.truncate`(warn) / `store.journal.compact`(info) / **`store.journal.compact.skip`(info，含仓库/AI 变更的段不压缩，D-159)** / `store.recover`(info) / `store.index.rebuild`(info) / `store.migrate`(info) / `store.snapshot.write`(debug) / `store.snapshot.gc`(info) / `store.snapshot.missing`(warn) / `store.auth.register`(info) / `store.auth.login`(info) / `store.auth.reject`(warn) / `store.auth.lock`(warn) / `store.abuse.suspect`(warn) / **`store.player.removed`(info，墓碑删除 `player.removed`，2026-09-19)** / **`store.warehouse.full`(warn，开箱超限丢弃，D-159)** / **`store.starter.issued`(info，新手套装生成，D-159)** / `store.error`(error) | B27~B33 ✅ / **D-159/D-161 🆕** |
...
- **F2 新增环境变量（✅ 已接线，D-158）**：`DL_ADMIN_USERS`（逗号分隔的**用户名**或 **publicId**/`playerId`；大小写不敏感；空/未配置 = 无账号级管理员）——读取点 = `server/admin.js` 的 `adminUsersOf(env)`（`server/index.js` 注入 `server/auth.js` 的用户名索引做用户名→playerId 比对）。
...
- **D-159/D-161/D-162 实例级覆盖缝（✅ 已接线；契约的一部分，不是内部细节）**：`start({boxSeed})` = 开箱 seed 的确定性序列（第 n 次调用 = `boxSeed+n−1`，**按调用次数推进（含被 4xx 拒绝的调用）**；D-162，缺省 `null` → `crypto.randomInt`）；`start({replayLimit})` / `start({config:{replayCacheSize}})`（既有）；`start({publicDir})`（F1）。数值上限 `warehouse.maxPerBucket`（500）与 `ai.maxPerPlayer`（100）在 `server/data/service-config.json`，schema 冻结比对（门禁项 4）。
- **D-159 新增文件必须登记**：`/^server\/(…|starter)\.js$/` → 6（`scripts/check-arch.js` LAYER_RULES）。**已登记。**
