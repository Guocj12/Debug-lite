...
12. **service-config.json（D-129/D-159/D-161，✅ 已存在）**：`{auth{...},session{...},config{maxSlots:3,slotIdPrefix},record{recentLimit:100},store{archiveCacheSize,snapshotCacheSize},journal{...},snapshot{retentionDays},replayCacheSize:64,pool{ttlDays,opponentCooldownHours},warehouse{maxPerBucket:500},ai{maxPerPlayer:100}}`。**D-159/D-161 新增 `warehouse.maxPerBucket` 与 `ai.maxPerPlayer`**（服务端强制：超限拒绝开箱 / 拒绝创建 AI）。`usernameMin/usernameMax/nicknameMax` **已被消费**（`nicknameMax` 在写入时夹到 ≤16）；`pool.ttlDays` 参数已留、**未启用**（默认 0 = 不过期）。校验与兜底同 §4.11。
